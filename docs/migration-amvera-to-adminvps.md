...
На Uptrends многие статические файлы отдавались по **HTTP/1.1**, из‑за чего растёт очередь запросов. В nginx на VPS включите:
...
Либо поставьте **Cloudflare** перед доменом (HTTP/2 и HTTP/3 к браузеру). Код сайта это не заменяет, но сильно ускоряет загрузку десятков мелких `/_next/static` чанков.
