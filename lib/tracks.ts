...
export interface Track {
  ...
  trackOrder?: number
  trackName: string
  /** Версия трека (Radio Edit, Remix и т.п.), опционально. */
  trackVersion: string
  artistName: string
  labelName: string
  genre: TrackGenre
...
export interface TrackRow {
  ...
  has_explicit_language?: boolean | null
...
export function rowToTrack(row: TrackRow): Track {
  ...
  return {
    ...
    aiLabeling: (() => {
      ...
      try {
        return parseTrackAiLabeling(JSON.parse(row.ai_labeling_json))
      } catch {
        return null
      }
    })(),
    ...
    hasExplicitLanguage:
      row.has_explicit_language === true ? true : row.has_explicit_language === false ? false : null,
...
export async function createTrack(data: CreateTrackInput): Promise<Track> {
  ...
  `,
    [
      ...
      track.aiLabeling ? JSON.stringify(track.aiLabeling) : null,
      track.createdAt,
      track.updatedAt,
    ]
  )
