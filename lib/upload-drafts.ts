...
export async function createUploadDraft(input: {
  ...
  return rowToDraft((await queryOne<UploadDraftRow>("SELECT * FROM upload_drafts WHERE id = ?", [id])) as UploadDraftRow)
}

export async function getUploadDraftById(id: string): Promise<UploadDraft | null> {
  const row = await queryOne<UploadDraftRow>("SELECT * FROM upload_drafts WHERE id = ?", [id])
  return row ? rowToDraft(row) : null
}

export async function getUploadDraftByBundleOrderId(orderId: string): Promise<UploadDraft | null> {
  const row = await queryOne<UploadDraftRow>(
    "SELECT * FROM upload_drafts WHERE bundle_order_id = ? ORDER BY updated_at DESC LIMIT 1",
    [orderId]
  )
  return row ? rowToDraft(row) : null
}
...
export async function getUploadDraftByAlbumId(albumId: string): Promise<UploadDraft | null> {
  const row = await queryOne<UploadDraftRow>(
    "SELECT * FROM upload_drafts WHERE album_id = ? ORDER BY created_at DESC LIMIT 1",
    [albumId]
  )
  return row ? rowToDraft(row) : null
}
...
export async function listUploadDrafts(params?: {
  ...
  const rows = await query<UploadDraftRow>(sql, [...values, limit])
...
export async function deleteUploadDraft(id: string): Promise<boolean> {
  ...
  const changes = await execute("DELETE FROM upload_drafts WHERE id = ?", [id])
  return changes > 0
}
...
export async function listDraftsForExpiryWindow(startIso: string, endIso: string): Promise<UploadDraft[]> {
  const rows = await query<UploadDraftRow>(
    `
