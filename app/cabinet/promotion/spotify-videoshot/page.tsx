...
function SpotifyVideoshotPageInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { t } = useI18n()
  const service = t.cabinet.promotion.spotifyVideoshot
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        router.replace("/cabinet/promotion/spotify-videoshot", { scroll: false })
      }
    })()
  }, [searchParams, router])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/cabinet/payments/spotify-videoshot/create", {
        method: "POST",
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {service.warningTitle && service.warningText ? (
              <div className="rounded-md border border-destructive/40 bg-destructive/5 p-3 text-sm">
                <p className="font-semibold text-destructive">{service.warningTitle}</p>
                <p className="mt-1 text-muted-foreground">{service.warningText}</p>
              </div>
            ) : null}
        ...
        <DialogContent className="w-[95vw] sm:max-w-[84rem] max-h-[95vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="spotify-videoshot-track-title">Трек для видеошота</Label>
              <div className="flex items-start gap-3 rounded-md border border-border p-3">
                <Checkbox
                  id="spotify-videoshot-without-track-selection"
                  checked={orderWithoutTrackSelection}
                  onCheckedChange={(checked) => setOrderWithoutTrackSelection(checked === true)}
                  disabled={loading}
                />
                <label htmlFor="spotify-videoshot-without-track-selection" className="text-sm font-normal leading-snug cursor-pointer">
              ...
              <select
                id="spotify-videoshot-track-title"
            ...
            <div className="space-y-2">
              <Label htmlFor="spotify-videoshot-comment">{formText.commentLabel}</Label>
              <Textarea
                id="spotify-videoshot-comment"
            ...
            <div className="space-y-2">
              <Label htmlFor="spotify-videoshot-contact-type">{formText.contactTypeLabel}</Label>
              <select
                id="spotify-videoshot-contact-type"
                className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                value={contactType}
            ...
            <div className="space-y-2">
              <Label htmlFor="spotify-videoshot-contact-value">{formText.contactValueLabel}</Label>
              <Input
                id="spotify-videoshot-contact-value"
                type="text"
            ...
            <p className="text-sm font-medium">
              {formText.totalLabel}: {SPOTIFY_VIDEOSHOT_PRICE_RUB} {formText.currencySuffix}
            </p>
