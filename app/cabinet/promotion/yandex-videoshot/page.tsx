...
function YandexVideoshotPageInner() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { t } = useI18n()
  const service = t.cabinet.promotion.yandexVideoshot
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        router.replace("/cabinet/promotion/yandex-videoshot", { scroll: false })
      }
    })()
  }, [searchParams, router])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/cabinet/payments/yandex-videoshot/create", {
        method: "POST",
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-muted-foreground">
                <p>{service.examplesUsageTitle}</p>
                <ul className="list-disc list-inside space-y-1">
                  {service.examplesUsageItems.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
                <p>{service.examplesRecommendationsTitle}</p>
                <ul className="list-disc list-inside space-y-1">
                  {service.examplesRecommendationsItems.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              </div>
        ...
        <DialogContent className="w-[95vw] sm:max-w-[84rem] max-h-[95vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="yandex-videoshot-track-title">Трек для видеошота</Label>
              <div className="flex items-start gap-3 rounded-md border border-border p-3">
                <Checkbox
                  id="yandex-videoshot-without-track-selection"
                  checked={orderWithoutTrackSelection}
                  onCheckedChange={(checked) => setOrderWithoutTrackSelection(checked === true)}
                  disabled={loading}
                />
                <label htmlFor="yandex-videoshot-without-track-selection" className="text-sm font-normal leading-snug cursor-pointer">
              ...
              <select
                id="yandex-videoshot-track-title"
            ...
            <div className="space-y-2">
              <Label htmlFor="yandex-videoshot-comment">{formText.commentLabel}</Label>
              <Textarea
                id="yandex-videoshot-comment"
            ...
            <div className="space-y-2">
              <Label htmlFor="yandex-videoshot-contact-type">{formText.contactTypeLabel}</Label>
              <select
                id="yandex-videoshot-contact-type"
                className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm"
                value={contactType}
            ...
            <div className="space-y-2">
              <Label htmlFor="yandex-videoshot-contact-value">{formText.contactValueLabel}</Label>
              <Input
                id="yandex-videoshot-contact-value"
                type="text"
            ...
            <p className="text-sm font-medium">
              {formText.totalLabel}: {YANDEX_VIDEOSHOT_PRICE_RUB} {formText.currencySuffix}
            </p>
