...
const YOOKASSA_PAYMENT_DESCRIPTION = "Создание видеоаватара для Яндекс Музыки"
...
export async function POST(request: NextRequest) {
  ...
  if (!shopId || !secretKey) {
    console.error(
      "[payments/yandex-videoavatar/create] Missing YOOKASSA env (YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY)"
    )
  ...
  const totalAmount = YANDEX_VIDEOAVATAR_PRICE_RUB.toFixed(2)
  ...
  const returnUrl = `${siteBase}/cabinet/promotion/yandex-videoavatar?payment=return&orderId=${encodeURIComponent(order.id)}`
  ...
  try {
    ...
    console.error("[payments/yandex-videoavatar/create] YooKassa request failed:", err)
  ...
  if (!res.ok) {
    console.error("[payments/yandex-videoavatar/create] YooKassa error:", res.status, data)
  ...
  if (!paymentId || !confirmationUrl) {
    console.error("[payments/yandex-videoavatar/create] YooKassa response missing id or confirmation_url:", data)
