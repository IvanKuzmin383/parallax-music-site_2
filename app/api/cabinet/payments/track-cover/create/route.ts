...
/** Текст услуги в ЮKassa (как в чеке/описании платежа) */
const YOOKASSA_PAYMENT_DESCRIPTION = "Создание обложки для релиза"
...
export async function POST(request: NextRequest) {
  ...
  if (!shopId || !secretKey) {
    console.error(
      "[payments/track-cover/create] Missing YOOKASSA env (YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY)"
    )
  ...
  const totalAmount = TRACK_COVER_PRICE_RUB.toFixed(2)
  ...
  const returnUrl = `${siteBase}/cabinet/promotion/track-cover?payment=return&orderId=${encodeURIComponent(order.id)}`
  ...
  try {
    ...
    console.error("[payments/track-cover/create] YooKassa request failed:", err)
  ...
  if (!res.ok) {
    console.error("[payments/track-cover/create] YooKassa error:", res.status, data)
    return NextResponse.json(
  ...
  if (!paymentId || !confirmationUrl) {
    console.error("[payments/track-cover/create] YooKassa response missing id or confirmation_url:", data)
