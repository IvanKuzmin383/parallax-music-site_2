...
const YOOKASSA_PAYMENT_DESCRIPTION = "Видеошот для Spotify"
...
export async function POST(request: NextRequest) {
  ...
  if (!shopId || !secretKey) {
    console.error(
      "[payments/spotify-videoshot/create] Missing YOOKASSA env (YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY)"
    )
  ...
  const totalAmount = SPOTIFY_VIDEOSHOT_PRICE_RUB.toFixed(2)
  ...
  try {
    ...
    console.error("[payments/spotify-videoshot/create] YooKassa request failed:", err)
  ...
  if (!res.ok) {
    console.error("[payments/spotify-videoshot/create] YooKassa error:", res.status, data)
  ...
  if (!paymentId || !confirmationUrl) {
    console.error("[payments/spotify-videoshot/create] YooKassa response missing id or confirmation_url:", data)
